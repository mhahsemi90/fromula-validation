...
import {getOperandFromMainList} from "../IntermediateFrame/getElementFromMainList.js";

const addExpressionToBlocks = (blocks, expression) => {
    let operator = {};
    switch (expression.type) {
        case ExpressionType.VARIABLE_EXPRESSION:
            operator = getOperandFromMainList(expression.idName);
            blocks.push(
                new Block(BlockType.VARIABLE, operator.title, operator.enTitle, operator.code)
            );
            break;
